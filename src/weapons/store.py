...
class StoreWeapons:
    ...
    def showcase(self, weapon_attributes: List[Enum] = None ) -> List[BaseWeapon]:
        ...
        for weapon in self.filtered_weapons:
            ...
            critical = '/'.join(str(threat) for threat in weapon.weapon_threat) if isinstance(weapon.weapon_threat,list) else str(weapon.weapon_threat)
            weapon_range = '/'.join(str(range) for range in weapon.range) if isinstance(weapon.range,list) else str(weapon.range)
            weapon_type = '/'.join(str(type) for type in weapon.type) if isinstance(weapon.type,list) else str(weapon.type)
            weight = weapon.weight
            name = weapon.name
            data.append([name,price,damage,critical,weapon_range,weapon_type,weight])
        ...
        table = tabulate.tabulate(data, headers=headers,tablefmt="rounded_grid",stralign="center")
        print(table)
